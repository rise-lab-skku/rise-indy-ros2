            ...
            if move["name"] == name:
                ...
                for waypoint in move["wpList"]:
                    try:
                        ...
                        round_position = [round(position, round_num) for position in
                                          data["wpList"][waypoint["id"]]["q"]]
        ...
        for move in data["moveList"]:
            if move["name"] == name:
                ...
                for waypoint in move["wpList"]:
                    try:
                        move_data["waypoint_active"].append(enable)
                        converting_point = [x * 1000 if i < 3 else x for i, x in
                                            enumerate(data["wpList"][waypoint["id"]]["p"])]
                        round_position = [round(position, round_num) for position in converting_point]
                        round_joint_pos = [round(position, round_num) for position in
                                           data["wpList"][waypoint["id"]]["q"]]
                        move_data["waypoints"].append(round_position)
            ...
            if move["name"] == name:
                ...
                for waypoint in move["wpList"]:
                    try:
                        ...
                        round_position = [round(position, round_num) for position in
                                          data["wpList"][waypoint["id"]]["q"]]
                        round_origin = [round(position, round_num) for position in
                                        move["wpListOrigin"][0]["startPose"]["q"]]
                        move_data["waypoints"].append(round_position)
            ...
            if move["name"] == name:
                ...
                for waypoint in move["wpList"]:
                    try:
                        ...
                        move_data["waypoint_active"].append(enable)
                        converting_origin = [x * 1000 if i < 3 else x for i, x in
                                             enumerate(move["wpListOrigin"][0]["startPose"]["p"])]
    ...
    def convert_motion(self, robot_ip_address):
        ...
        for val in input_data["program"][1]["varList"]:  # 변수등록
            if val["name"][0] == 'B' and val["type"] == 5:
                B_variaable = True
                output_data["Program"][2]["local_list"].append(
                    ["INT", val["name"], int(''.join(filter(str.isdigit, val["name"]))), "LOCAL"])
            if val["name"][0] == 'M' and val["type"] == 5:
                output_data["Program"][2]["modbus_list"][1]["variable_list"].append(
                    ["WRITE_REGISTER", val["name"] + "_WRITE", int(''.join(filter(str.isdigit, val["name"]))), "LOCAL"])
                output_data["Program"][2]["modbus_list"][0]["variable_list"].append(
                    ["READ_REGISTER", val["name"] + "_READ", int(''.join(filter(str.isdigit, val["name"]))), "LOCAL"])
...
read_json_name = data["Input_Name"]
converting_name = data["Out_Name"] + "." + data["Robot_Model"] + ".json"
robot_ip = data["Robot_IP"]
robot_name = data["Robot_Model"]
robot_dof = data["Robot_DOF"]

robot_name_Config = robot_name
robot_name_Config = 'I' + robot_name_Config[1:]

if data["Convert_Modbus"]:
    ipadress = robot_ip
...
if data["Convert_Tool"]:
    with open("conty2/" + read_json_name, "r") as file:
...
if True:
    ...
    with open("/home/user/release/IndyDeployment/ProgramScripts/" + converting_name, "w") as output_file:
